...
static void get_textures(t_cub *cub, int tx)
{
    ...
    cub->tx_data[tx]->ptr = mlx_xpm_file_to_image(cub->mlx->mlx, cub->map->textures_paths[tx], &cub->tx_data[tx]->width, &cub->tx_data[tx]->height);
    ...
    {
		cub->tx_data[tx]->texture[y] = (int *)malloc(sizeof(int) * cub->tx_data[tx]->width);
   		x = -1;
		while(++x < cub->tx_data[tx]->width)
			cub->tx_data[tx]->texture[y][x] = cub->tx_data[tx]->data[y * cub->tx_data[tx]->width + x];
	}
